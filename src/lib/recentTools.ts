 ...
 * 记录工具使用
 */
export function recordToolUsage(toolId: string, toolName: string | { zh: string; en: string }, toolPath: string, icon?: any) {
  if (typeof window === 'undefined') return;
  ...
  try {
    ...
    // 处理 toolName 可能是对象的情况
    const nameString = typeof toolName === 'string' ? toolName : toolName.zh || toolName.en || toolId;
