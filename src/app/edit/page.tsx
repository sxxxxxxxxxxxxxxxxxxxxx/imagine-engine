...
export default function EditPage() {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      // 目标：确保最终图片大小 < 500KB（inpaint需要两张图，所以单张要更小）
      const targetMaxSize = selectedTool === 'inpaint' ? 400 * 1024 : 500 * 1024; // inpaint时更严格
      
      if (originalSize > targetMaxSize) {
        console.log(`📦 图片较大(${Math.round(originalSize / 1024)}KB)，开始压缩...`);
        try {
          // 根据原始大小和工具类型动态调整压缩参数
          ...
          console.log(`✅ 压缩完成: ${Math.round(imageDataUrl.length / 1024)}KB`);
          
          // 如果压缩后仍然太大，进一步压缩
          if (imageDataUrl.length > targetMaxSize) {
            console.log(`⚠️ 仍然过大，进一步压缩...`);
            ...
            console.log(`✅ 二次压缩完成: ${Math.round(imageDataUrl.length / 1024)}KB`);
          }
        } catch (compressError) {
          console.warn('⚠️ 压缩失败，使用原图:', compressError);
        }
      }
      ...
      if (selectedTool === 'inpaint' && maskCanvasRef.current) {
        ...
        // mask目标大小：< 200KB（黑白图可以压缩得很小）
        const maskTargetSize = 200 * 1024;
        
        if (maskSize > maskTargetSize) {
          console.log(`📦 Mask较大(${Math.round(maskSize / 1024)}KB)，开始压缩...`);
          try {
            // mask是黑白图，使用更激进的压缩
            // 先压缩尺寸，再降低质量
            let maxDim = 800;
            ...
            console.log(`✅ Mask压缩完成: ${Math.round(maskDataUrl.length / 1024)}KB`);
            
            // 如果仍然太大，进一步压缩
            if (maskDataUrl.length > maskTargetSize) {
              console.log(`⚠️ Mask仍然过大，进一步压缩...`);
              maskDataUrl = await compressImage(maskDataUrl, 512, 0.4);
              console.log(`✅ Mask二次压缩完成: ${Math.round(maskDataUrl.length / 1024)}KB`);
            }
          } catch (compressError) {
            console.warn('⚠️ Mask压缩失败，使用原图:', compressError);
          }
        }
